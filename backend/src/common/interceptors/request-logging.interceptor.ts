...
export class RequestLoggingInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        const isEnabled = this.config.get('REQUEST_LOGGING_ENABLED', 'true') === 'true';
        if (!isEnabled) {
            return next.handle();
        }
        ...
        return next.handle().pipe(
            tap({
                ...
                error: (err) => this.log(context, method, path, ip, userAgent, start, err),
            })
        );
    }

    private async log(context: ExecutionContext, method: string, path: string, ip: string, userAgent: string, start: number, error?: any) {
        ...
        // Sampling
        const sampleRate = parseFloat(this.config.get('REQUEST_LOGGING_SAMPLE_RATE', '1.0'));
        ...
        try {
            ...
            if (durationMs > 1000) {
                this.logger.warn(`Slow Request: ${method} ${path} took ${durationMs}ms`);
            }
        } catch (e: any) {

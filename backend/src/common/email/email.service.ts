...
export class EmailService {
    ...
    constructor(private configService: ConfigService) {
        this.provider = this.configService.get<string>('EMAIL_PROVIDER', 'ethereal');
        ...
        switch (this.provider) {
            case 'mailtrap':
                this.transporter = nodemailer.createTransport({
                    host: this.configService.get('MAILTRAP_HOST', 'sandbox.smtp.mailtrap.io'),
            ...
            case 'ses':
                ...
                this.logger.warn('SES provider selected but not implemented. Falling back to console logging.');
                break;

            default:
                this.logger.warn(`Unknown provider: ${this.provider}. Falling back to console logging.`);
        }
    }
    ...
    async send(to: string, subject: string, html: string, text?: string) {
        const from = this.configService.get('EMAIL_FROM', 'noreply@serviceflow.com');
        ...
        try {
            ...
            if (this.transporter) {
                ...
                this.logger.log(`Email sent via Nodemailer (${this.provider}): ${info.messageId}`);
            ...
            // Fallback log
            this.logger.warn(`Email provider not active. content: Subject: ${subject}`);

  ...
  async detectRisk(
    ...
    if ((user.workerProfile?.bankDetails as any)?.accountNumberHash) {
      const duplicateCount = await (this.prisma.bankDetails as any).count({
        where: {
          accountNumberHash: (user.workerProfile!.bankDetails as any)
            .accountNumberHash,
    ...
    const uniqueIps = new Set(
      recentLogins.map((l: any) => l.ipAddress).filter(Boolean),
    );
    if (uniqueIps.size > 3) {
      score += 10;
    ...
    // 5. Job Arrival Fraud Signals (Mock Location / Low Accuracy)
    const recentJobs = await this.prisma.job.findMany({
      where: { workerId: userId, status: 'ARRIVED' },
      take: 5,
      orderBy: { arrivedAt: 'desc' },
    });

    const mockArrivals = recentJobs.filter((j: any) => j.arrivalIsMock).length;
    if (mockArrivals > 0) {
      score += mockArrivals * 40; // Very high penalty for mock locations
      flags.push('MOCK_LOCATION_DETECTED');
    }

    const lowAccuracyArrivals = recentJobs.filter(
      (j: any) => (j.arrivalAccuracyMeters || 0) > 100,
    ).length;
    if (lowAccuracyArrivals >= 2) {
      score += 15;

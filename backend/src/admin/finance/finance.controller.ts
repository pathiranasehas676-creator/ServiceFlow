...
export class FinanceController {
    ...
    async getWallets(@Query('page') page = 1, @Query('limit') limit = 20, @Query('q') q = '') {
    ...
    async getTransactions(@Query('page') page = 1, @Query('limit') limit = 20, @Query('walletId') walletId?: string) {
        return this.financeService.getTransactions(Number(page), Number(limit), walletId);
    }
    ...
    async adjustBalance(
        ...
        return this.financeService.adjustBalance(walletId, body.amountCents, body.reason, req.user.id);
    }
}

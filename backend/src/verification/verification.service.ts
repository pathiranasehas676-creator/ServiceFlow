...
export class VerificationService {
    ...
    async getVerificationStatus(userId: string) {
        ...
        if (bankDetails) {
            ...
            else bankStatus = 'PENDING'; // Can't easily distinguish REJECTED without history or schema change
        }
        ...
        return {
            ...
            bankStatus,
            idRejectionReason: idVerification?.status === 'REJECTED' ? idVerification.rejectionReason : null,
            bankRejectionReason: null, // Would need to fetch from history
            missingSteps
        };
    }

    async startIdUpload(userId: string, files: { type: string, size: number, mimeType: string }[]) {
        // In a real implementation, this would generate signed URLs for S3/MinIO
        // For now, we'll return mock URLs or just accept the keys if client uploads directly
        // Requirement says "returns presigned PUT URLs"

        const uploads = files.map(f => ({
            type: f.type,
            key: `verifications/${userId}/${Date.now()}-${f.type}-${Math.random().toString(36).substring(7)}`,
            url: `https://fake-s3-subdomain.example.com/upload?key=...` // Mock for now as S3 service isn't injected
        }));

        return { uploads };
    }

    async submitIdVerification(userId: string, data: { frontImageKey: string; backImageKey?: string; selfieKey?: string; documentType: string; documentNumber?: string }) {
    ...
    async submitBankDetails(userId: string, data: { bankName: string; accountNumber: string; accountName: string; branchCode?: string; swiftCode?: string }) {
        ...
        const existing = await this.prisma.bankDetails.findUnique({ where: { accountNumberHash: accountHash } });
    ...
    async getVerificationQueue(type: 'id' | 'bank', status: 'PENDING' | 'APPROVED' | 'REJECTED', page = 1, limit = 20, q?: string) {
        const skip = (page - 1) * limit;

        if (type === 'id') {
            const where: any = { status };
            if (q) {
                where.workerProfile = { user: { OR: [{ email: { contains: q } }, { fullName: { contains: q } }] } };
            }
    ...
    async approveIdVerification(id: string, adminId: string) {
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (verification.status === 'APPROVED') return { success: true, userId: verification.workerProfile.userId };
            ...
            await this.logAudit(tx, adminId, 'APPROVE', 'IdVerification', id, 'Approved ID verification');
    ...
    async rejectIdVerification(id: string, adminId: string, reason: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            await this.logAudit(tx, adminId, 'REJECT', 'IdVerification', id, `Rejected ID: ${reason}`);
    ...
    async approveBankVerification(id: string, adminId: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            await this.logAudit(tx, adminId, 'APPROVE', 'BankDetails', id, 'Approved bank details');
    ...
    async rejectBankVerification(id: string, adminId: string, reason: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            await this.logAudit(tx, adminId, 'REJECT', 'BankDetails', id, `Rejected bank: ${reason}`);
    ...
    private async logHistory(tx: any, data: { userId: string, action: string, type: string, newStatus?: string, previousStatus?: string, reason?: string, adminId?: string }) {
        return tx.verificationHistory.create({ data });
    }

    private async logAudit(tx: any, userId: string, action: string, entityType: string, entityId: string, details: string) {
